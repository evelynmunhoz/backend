...
> Obs: Em arrays associativos, nós trocamos os n° do índice por nomes (Chaves/Keys). A setinha => significa "recebe".

**Arrays Multidimensionais** (Banco de dados na memória)

É aqui que o "Backend" começa de verdade. O Array multidimensional é o formato como os bancos de dados chegar como respostas as solicitações feitas pela API.
...
São usadas em filtraem de Mapeamento de dados de um array 
...
- `print_r` 
Função usada para exibir informações sobre uma variével de forma legível em limguagem natural
...
Usamos o operador de nulidade (Coalescência nula) para verificar se o valor da variével não é `null`, se caso for `null` atribuimos um valor para evitar erros no script 
 ...
 **Exemplo de uso** :
 ...
 $nome = $_POST["nome"] ?? "";
 //se $_POST["nome] não exitir, use uma string vazia.
...
#### Presrvação de Estado em formulários (*sticky form*)

A técnica do *sticky form* consiste em imprimir de volta o valor no atributo "value" do input, od dados que usuário acaba de digitar, os valores são devolvidos aos inputs caso ocorra algum erro de validação de dados no envio.
...
### Semana 7 - Segurança no BackEnd - Santização, Validação e Proteção contra XSS 

**1° Mandamento do Desenvolvimento BackEnd** 
...
#### **A Triáde de Defesa: Validação, Sanitização e Escapamento**

1. **Validção**: Verifica se o dado recebido atende aos requisitos exatos do sistema (tipo,tamanho,formato). 
